...
- (void)zhuXiaoButtonClik
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:Zhuxiaodenglu object:nil userInfo:nil];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        ...
        self.switc.onTintColor = KHexColor(@"#ff5873");
        //self.switc.tintColor = [UIColor grayColor];KHexColor(@"#ebebeb");
        self.switc.thumbTintColor = KHexColor(@"#ffffff");
        [self.switc addTarget:self action:@selector(switcChange:) forControlEvents:UIControlEventValueChanged];
        [cell addSubview:_switc];
...
- (void)switcChange:(id)sender
{
    UISwitch *mySwitch = (UISwitch *)sender;
    if (mySwitch.isOn)
    {
        [self setqueWithStatus:@"1"];
    }
    else
    {
        [self setqueWithStatus:@"0"];
    }
}
#pragma mark ------------------------------------------------------------------ASURLConnection ------------------------------------------------------------
-(void)setqueWithStatus:(NSString *)statusString{
    ASUserInfoModel *user=[[ASUserInfoModel alloc]init];
    NSDictionary *json=@{@"user_id":[NSNumber numberWithInteger:[user getUserMessageFromLocation].userId],@"status":statusString};
    
    [ASURLConnection requestAFNJSon:json method:@"jumper.user.pushstatus.update" view:self.view version:@"" completeBlock:^(NSData *data, id responseObject) {
