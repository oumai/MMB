...
@interface  TemperatureRecordModel  : NSObject
...
@interface TemperatureRecordModel ()

@end

@implementation TemperatureRecordModel
...
@interface TemperatureRecordCustomCell : UITableViewCell
...
@interface TemperatureRecordCustomCell ()
...
@implementation TemperatureRecordCustomCell
...
@interface TemperatureRecordVC ()<UITableViewDataSource,UITableViewDelegate,DateDelegate>
...
-(void)getTemPreture:(NSString *)dateString{
    ...
    [ASURLConnection requestAFNJSon:@{@"user_id":[NSNumber numberWithInteger:[self.user getUserMessageFromLocation].userId],@"date":dateString} method:kGetTemRequestRecord view:nil version:@"" completeBlock:^(NSData *data, id responseObject) {
        ...
        if ([[ASURLConnection getMsg:responseObject]isEqual:@1]) {
            ...
            else
            {
                ...
                {
                    TemperatureRecordModel *model = [[TemperatureRecordModel alloc] initWithDictionary:dataArr[i]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TemperatureRecordCustomCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (nil == cell) {
        cell = [[TemperatureRecordCustomCell alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:identifier];
    }
...
- (void)requestData
{
    [ASURLConnection requestAFNJSon:@{@"user_id":[NSNumber numberWithInteger:[self.user getUserMessageFromLocation].userId],@"date":self.dayStr,@"type":@4} method:@"renhe.system.healthy1Data" view:self.view version:@"/v3" completeBlock:^(NSData *data, id responseObject) {
