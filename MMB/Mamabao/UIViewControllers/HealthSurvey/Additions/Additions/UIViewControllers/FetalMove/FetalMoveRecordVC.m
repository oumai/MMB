...
@interface  FetalMoveRecordModel  : NSObject
...
@interface FetalMoveRecordModel ()

@end

@implementation FetalMoveRecordModel
...
@interface FetalMoveRecordCustomCell : UITableViewCell
...
@interface FetalMoveRecordCustomCell ()
...
@implementation FetalMoveRecordCustomCell
...
#import "FetalMoveRecordVC.h"

@interface FetalMoveRecordVC ()<UITableViewDataSource,UITableViewDelegate,DateDelegate>
...
@implementation FetalMoveRecordVC
        ...
        if ([[ASURLConnection getMsg:responseObject] isEqualToNumber:@1]) {
            ...
            else
            {
                for (int i = 0; i <dataArr.count; i++) {
                    FetalMoveRecordModel *model = [[FetalMoveRecordModel alloc] initWithDictionary:dataArr[i]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    FetalMoveRecordCustomCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (nil == cell) {
        cell = [[FetalMoveRecordCustomCell alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:identifier];
    }
...
- (void)requestData
{
    [ASURLConnection requestAFNJSon:@{@"user_id":[NSNumber numberWithInteger:[self.user getUserMessageFromLocation].userId],@"date":self.dayStr,@"type":@8} method:@"renhe.system.healthy1Data" view:self.view version:@"/v3" completeBlock:^(NSData *data, id responseObject) {
        [_dataArray removeAllObjects];
        if ([[ASURLConnection getMsg:responseObject] isEqualToNumber:@1]) {
            NSArray *dataArr = [[[ASURLConnection doDESDecrypt:responseObject] objectFromJSONString] objectForKey:@"data"];
            if (dataArr.count == 0) {
