...
@interface  BloodSugarRecordModel  : NSObject
...
@interface BloodSugarRecordModel ()

@end

@implementation BloodSugarRecordModel
...
@interface BloodSugarRecordCustomCell : UITableViewCell
...
@interface BloodSugarRecordVC ()<UITableViewDataSource,UITableViewDelegate,DateDelegate>
...
@implementation BloodSugarRecordVC
        ...
        if ([[ASURLConnection getMsg:responseObject] isEqualToNumber:@1]) {
            ...
            else
            {
                for (int i = 0; i <dataArr.count; i++) {
                    BloodSugarRecordModel *model = [[BloodSugarRecordModel alloc] initWithDictionary:dataArr[i]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BloodSugarRecordCustomCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (nil == cell) {
        cell = [[BloodSugarRecordCustomCell alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:identifier];
    }
...
- (void)requestData
{
    [ASURLConnection requestAFNJSon:@{@"user_id":[NSNumber numberWithInteger:[self.user getUserMessageFromLocation].userId],@"date":self.dayStr,@"type":@7} method:@"renhe.system.healthy1Data" view:self.view version:@"/v3" completeBlock:^(NSData *data, id responseObject) {
        [_dataArray removeAllObjects];
        if ([[ASURLConnection getMsg:responseObject] isEqualToNumber:@1]) {
            NSArray *dataArr = [[[ASURLConnection doDESDecrypt:responseObject] objectFromJSONString] objectForKey:@"data"];
            if (dataArr.count == 0) {
