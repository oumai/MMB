...
@interface  WeightRecordModel  : NSObject
...
@interface WeightRecordModel ()

@end

@implementation WeightRecordModel
...
@interface WeightRecordCustomCell : UITableViewCell
...
@interface WeightRecordCustomCell ()
...
@implementation WeightRecordCustomCell
...
#import "WeightRecordVC.h"

@interface WeightRecordVC ()<UITableViewDataSource,UITableViewDelegate,DateDelegate>
        ...
        if ([[ASURLConnection getMsg:responseObject] isEqualToNumber:@1]) {
            ...
            else
            {
                for (int i = 0; i <dataArr.count; i++) {
                    WeightRecordModel *model = [[WeightRecordModel alloc] initWithDictionary:dataArr[i]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    WeightRecordCustomCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (nil == cell) {
        cell = [[WeightRecordCustomCell alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:identifier];
    }
...
- (void)requestData
{
    [ASURLConnection requestAFNJSon:@{@"user_id":[NSNumber numberWithInteger:[self.user getUserMessageFromLocation].userId],@"date":self.dayStr,@"type":@5} method:@"renhe.system.healthy1Data" view:self.view version:@"/v3" completeBlock:^(NSData *data, id responseObject) {
        [_dataArray removeAllObjects];
        if ([[ASURLConnection getMsg:responseObject] isEqualToNumber:@1]) {
