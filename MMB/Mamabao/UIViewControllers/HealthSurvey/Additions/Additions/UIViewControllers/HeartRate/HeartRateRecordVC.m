...
@interface  HeartRateRecordModel  : NSObject
...
@interface HeartRateRecordModel ()

@end

@implementation HeartRateRecordModel
...
@interface HeartRateRecordCustomCell : UITableViewCell
...
@interface HeartRateRecordCustomCell ()
...
@implementation HeartRateRecordCustomCell
...
#import "HeartRateRecordVC.h"

@interface HeartRateRecordVC ()<UITableViewDataSource,UITableViewDelegate,DateDelegate>
...
@implementation HeartRateRecordVC
    ...
    [ASURLConnection requestAFNJSon:@{@"user_id":[NSNumber numberWithInteger:[_user getUserMessageFromLocation].userId],@"date":dateString} method:kGetTemRequestRecord view:nil version:@"" completeBlock:^(NSData *data, id responseObject) {
        ...
        if ([[ASURLConnection getMsg:responseObject]isEqual:@1]) {
            ...
            else
            {
                ...
                {
                    HeartRateRecordModel *model = [[HeartRateRecordModel alloc] initWithDictionary:dataArr[i]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    HeartRateRecordCustomCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (nil == cell) {
        cell = [[HeartRateRecordCustomCell alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:identifier];
    }
...
- (void)requestData
{
    [ASURLConnection requestAFNJSon:@{@"user_id":[NSNumber numberWithInteger:[self.user getUserMessageFromLocation].userId],@"date":self.dayStr,@"type":@3} method:@"renhe.system.healthy1Data" view:self.view version:@"/v3" completeBlock:^(NSData *data, id responseObject) {
