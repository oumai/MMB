...
@property (nonatomic, strong) TTTAttributedLabel *label8;
...

{
    if (self == [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        //self.label6.text = [NSString stringWithFormat:@"%@至%@",,];
...
@interface PayOrderVC ()<UITableViewDataSource,UITableViewDelegate,FDAlertViewDelegate,ASPayDelegate>
...
@property (nonatomic, strong) ASPay *pay;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        NSString *package_name = [self.orderDic objectForKey:@"package_name"];
        ...
        cell.label5.text = mobile;
        cell.label6.text = [NSString stringWithFormat:@"%@至%@",startDateTime,endDateTime];
        return cell;
...
- (void)requestData
{
    [ASURLConnection requestAFNJSon:@{@"order_id":self.order_id} method:@"renhe.orderService.orderDetail" view:self.view version:@"/v3" completeBlock:^(NSData *data, id responseObject) {
        if ([[ASURLConnection getMsg:responseObject] isEqualToNumber:@1])
        {
            ...
            self.orderProject = [NSString stringWithFormat:@"%@-%@",[self.orderDic objectForKey:@"entity_name"],[self.orderDic objectForKey:@"package_name"]];
            [_tableView reloadData];
        }
    } errorBlock:^(NSError *error) {
    }];
