...
- (void)goYuYueButtonClick
{
    ...
    yue.nanny_id  = [[self.dataDic objectForKey:@"id"] stringValue];
    yue.entity_id = self.entity_id;
    [self.navigationController pushViewController:yue animated:YES];
}

- (void)requestData
{
    [ASURLConnection requestAFNJSon:@{@"nanny_id":self.nanny_id} method:@"renhe.orderService.nannyDetail" view:self.view version:@"/v3" completeBlock:^(NSData *data, id responseObject) {
