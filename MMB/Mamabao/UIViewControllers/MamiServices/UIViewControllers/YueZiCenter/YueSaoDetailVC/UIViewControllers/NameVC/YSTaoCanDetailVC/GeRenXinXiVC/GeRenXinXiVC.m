...
- (void)submitButtonClick
{
    ...
    if (self.nameTF.text.length>0&&self.phoneTF.text.length>0&&self.addressTF.text.length>0&&self.startTF.text.length>0&&self.endTF.text.length>0)
    {
        NSDictionary *jsonDic = @{@"user_id":[NSNumber numberWithInteger:[self.user getUserMessageFromLocation].userId],
                                  @"isPerson":@1,
                                  @"entity_id":self.entity_id,
                                  @"package_id":self.nanny_id,
    ...
    {
        k_yjHUD(@"请输入结束时间", self.view)
    }
}
- (void)afterDelayClick
{
    YSPayOrderVC *ysp = [YSPayOrderVC new];
    ysp.order_id = [self.order_idDic objectForKey:@"id"];
    [self.navigationController pushViewController:ysp animated:YES];
}
@end
