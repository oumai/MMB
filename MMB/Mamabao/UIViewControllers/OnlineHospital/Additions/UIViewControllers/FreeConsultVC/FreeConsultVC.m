...
- (void)chooseKeshiButtonClick
{
    ...
    [simanPopoverViewController setSureBlock:^(KeShiVC *keshi)                                   //确定方法
    {
        [self.simanPopoverController dismissPopoverAnimated:YES completion:^{
        }];
        self.titleLabe.text = keshi.name;               //传入科室
        self.majorId = [keshi.major_id integerValue];   //传入科室ID
   }];
...
{
    if(!self.majorId &&(self.payType == payTypeFree || self.payType == payTypeFamily) )
    {
        k_NSlogHUD(@"请选择您要咨询的科室")
    }
    else if ([NSString isBlankString:self.bingQingTV.text] == YES)
    {
        k_NSlogHUD(@"请输入咨询问题")
    }
    else if ([NSString countWord:self.bingQingTV.text] < 18)
    {
        k_NSlogHUD(@"字数不小于18字")
    }
    else if (self.bingQingTV.text.length>180){
        k_NSlogHUD(@"字数太长了!")
    }
    else
    {
        ...
        {
            if (self.payType == payTypeFree )
            {
                NSDictionary *json = @{@"user_id":[NSNumber numberWithInteger:[self.user getUserMessageFromLocation].userId],
                                       @"major_id":[NSString stringWithFormat:@"%ld",(long)self.majorId],
                                       @"doctor_id":[NSNumber numberWithInteger:[self.doctorId integerValue]],
                                       @"content":self.bingQingTV.text,
                                       @"file_url":@"",
                                       @"length":@0,
                                       @"free_type":@0,
                                       @"pay_consult_id":@0};
                [self addContent:json];
            }
            else if (self.payType == payTypeFamily)
            {
                NSDictionary *json = @{@"user_id":[NSNumber numberWithInteger:[self.user getUserMessageFromLocation].userId],
                                       @"major_id":[NSString stringWithFormat:@"%ld",(long)self.majorId],
                                       @"doctor_id":[NSNumber numberWithInteger:[self.doctorId integerValue]],
            ...
            else if(self.payType == payTypeConsult)
            {
                NSDictionary *json = @{@"problem_id":self.groupID,
                                       @"content":self.bingQingTV.text,
                                       @"file_url":@"",
                                       @"length":@"",
                                       @"talker":@1};
                [self continueConsult:json];
            }
            else if(self.payType == payTypeYiZhen)
            {
                NSDictionary *json = @{@"user_id":[NSNumber numberWithInteger:[self.user getUserMessageFromLocation].userId],
                                       @"major_id":[NSString stringWithFormat:@"%ld",(long)self.majorId],
                                       @"doctor_id":[NSNumber numberWithInteger:[self.doctorId integerValue]],
...
-(void)addContent:(NSDictionary *)json{
    [ASLoadingView showActivity:self.view];
    [ASURLConnection requestAFNJSon:json method:kAddConsult view:nil version:@"/v3"  completeBlock:^(NSData *data, id responseObject) {
        [ASLoadingView hidenActivity:self.view];
        if ([[ASURLConnection getMsg:responseObject]isEqual:@1])
        {
            [self.bingQingTV resignFirstResponder];
            NSDictionary *chatDic = [[[ASURLConnection doDESDecrypt:responseObject] objectFromJSONString] objectForKey:@"data"][0];
            ConsultingChat *consult = [ConsultingChat new];
            consult.detailId = [[chatDic objectForKey:@"id"] integerValue];
            [self.navigationController pushViewController:consult animated:YES];
            //[[NSNotificationCenter defaultCenter]postNotificationName:@"moneyConsult" object:nil];/**付费咨询逻辑需要的刷新通知**/
...
-(void)continueConsult:(NSDictionary *)json{
    [ASURLConnection requestAFNJSon:json method:kContinueConsult view:nil version:@"/v3" completeBlock:^(NSData *data, id responseObject) {
        [ASLoadingView hidenActivity:self.view];
        
        if ([[ASURLConnection getMsg:responseObject]isEqual:@1]) {
            
            NSDictionary *chatDic = [[[ASURLConnection doDESDecrypt:responseObject] objectFromJSONString] objectForKey:@"data"][0];
            [self.bingQingTV resignFirstResponder];                                 //跳转到咨询记录
            ConsultingChat *consult = [ConsultingChat new];
            consult.detailId = [[chatDic objectForKey:@"id"] integerValue];
            [self.navigationController pushViewController:consult animated:YES];
