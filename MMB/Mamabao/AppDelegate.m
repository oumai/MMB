...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    //[NSThread sleepForTimeInterval:2]; //让启动页面延时
    self.user = [[ASUserInfoModel alloc] init];
    [[UINavigationBar appearance] setBarTintColor:KHexColor(@"#ff5873")];//顶栏颜色#ff6363    KHexColor(@"#ff5873")  KHexColor(@"#ff637e")
    self.window = [[UIWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
    self.window.backgroundColor = [UIColor whiteColor];
    
    AppLaunchManager *launchManager = [AppLaunchManager sharedManager];
    [launchManager setLauchingRootViewControllerWithCoreNewFeature:^(CoreNewFeatureVC *coreNewFeatureVC) {
        self.window.rootViewController = coreNewFeatureVC;
    }];
    
    [launchManager setLaunchRootViewControllerWithTabbarController:^(UIViewController *tabBarController) {
        ...
        else
        {
            ...
            self.navLogin = [[UINavigationController alloc] initWithRootViewController:login];
