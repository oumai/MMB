...
+ (BOOL)validateMobile:(NSString *)mobileNum
{
     ...
     */
//   NSString *mobileNoRegex = @"^1((3\\d|5[0-35-9]|8[025-9]|7\\d)\\d)\\d{7}$";//除4以外的所有个位整数，不能使用[^4,\\d]匹配，这里是否iOS Bug?
// 
...
//    return (ret);
    ...
    BOOL ret;
    if ([mobileNum stringByReplacingOccurrencesOfString:@" " withString:@""].length!=11) {
        ret = NO;
    }else{
        ret = YES;
    }
