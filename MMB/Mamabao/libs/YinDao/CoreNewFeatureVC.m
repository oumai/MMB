...
-(void)imageViewsPrepare{
    ...
    [self.models enumerateObjectsUsingBlock:^(NewFeatureModel *model, NSUInteger idx, BOOL *stop) {
        ...
        if(idx == self.models.count -1) {
            ...
            [button addTarget:self action:@selector(goToHomePageVC:) forControlEvents:UIControlEventTouchUpInside];
            [imageV addSubview:button];
            
            [button mas_makeConstraints:^(MASConstraintMaker *make) {
                make.centerX.equalTo(imageV.mas_centerX);
                make.bottom.equalTo(imageV.mas_bottom).offset(-15.0f);
...
- (void)goToHomePageVC:(UIButton *)button {
    [self dismiss];
}
