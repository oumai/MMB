...
- (void)setup {
    ...
    self.dateFont = [UIFont systemFontOfSize:15.0f];
    self.weekdayFont = [UIFont systemFontOfSize:12.0f];
    self.weekdayTextColor = RGBA(83, 83, 83, 1);
}
...
- (UIButton*)buttonForDate:(NSDate*)date index:(NSUInteger)index {
    ...
    if ( [date isEqualToDate:[self dateWithoutTimeFromDate:[NSDate date]]]) {
...
//    b.contentVerticalAlignment = UIControlContentVerticalAlignmentCenter;
    ...
    [b addTarget:self action:@selector(dayButtonPressed:aanDate:) forControlEvents:UIControlEventTouchUpInside];

    return b;
}

- (void)dayButtonPressed:(UIButton*)sender aanDate:(NSDateComponents *)components {
    _lastSelectedButton.selected = NO;
    sender.selected = YES;
    _lastSelectedButton = sender;
    self.selectedDate = _days[sender.tag];
    [sender setTitleColor:self.selectedDateTextColor forState:UIControlStateSelected];
    [sender setBackgroundImage:self.selectedDateBackgroundImage forState:UIControlStateSelected];
    [self.delegate getdate:sender.titleLabel.text];
}
...
- (void)recenter {
    ...
    for (NSInteger i=fromIndex;i<=toIndex;i++) {
        ...
        for (NSUInteger j=0;j<days.count;j++) {
            ...
            b.frame = CGRectMake(_edgeInsets.left+(i-fromIndex)*self.frame.size.width+j*_dx+5,kWeekdayLabelHeight+3,_dx-10,_dx-10);
            b.layer.masksToBounds  =YES;
            b.layer.cornerRadius =(_dx-10)/2;
            if (b.selected) {
...
- (NSArray*)daysForWeekAtIndex:(NSInteger)i {
    ...
    NSInteger before = components.weekday - 1;
...
+ (NSArray*)weekdayTitlesWithLocaleIdentifier:(NSString*)localeIdentifier length:(NSUInteger)length uppercase:(BOOL)uppercase {
    ...
    for (NSString *s in df.shortWeekdaySymbols) {
        NSString *clipped = [s substringWithRange:NSMakeRange(0,2)];
        if (uppercase) {
